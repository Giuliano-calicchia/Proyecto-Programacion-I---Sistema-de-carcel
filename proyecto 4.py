...
def CreacionCarcel():
    ...
    d_carcel = tuple([int(i) for i in linea.split(",")]) #Tupla que contendra las dimensiones de la carcel.
    lectura.close()
...
def EscrituraArchivo(l_datos,d_carcel,l_ubicacion):
    ...
    registro_utilizadas.write(f"{presos_max},{celdas_ocupadas},{presos_max-celdas_ocupadas}")
...
def InformacionPrisioneros():
    ...
    t_presos = tuple([int(i) for i in linea.split(",")[:-1]]) #Tupla que contendra los legajos de los prisioneros
    lectura_legajos.close()
    lectura_crimenes = open("crimenes.txt","rt")
    linea = lectura_crimenes.readline()
    t_crimenes = tuple([i for i in linea.split(",")[:-1]]) #Tupla que contendra los crimenes cometidos por cada prisionero
    lectura_crimenes.close()
    lectura_tiempo = open("tiempo.txt","rt")
    linea = lectura_tiempo.readline()
    t_tiempo = tuple([int(i) for i in linea.split(",")[:-1]]) #Tupla que contendra los tiempos de condena de cada prisionero
...
def AccesoLegajo(t_presos,l_datos):
    ...
    while True:
        try:
            ...
            registro_accesos.write(f"{op},") #Se registra el legajo en el archivo.
